// Constants for Japanese jazz content

// Hashtags to filter for Japanese jazz content
export const HASHTAGS = [
  '#jjazz',
  '#japanesejazz',
  '#jazzkissa',
  '#jazzukissa',
  '#nihonnojazzu',
  '#nihonjazz',
]

// Japanese keywords related to jazz
export const JAPANESE_KEYWORDS = [
  'ジャズ喫茶', // Jazz kissa
  '日本のジャズ', // Japanese jazz
  'jazz kissa',

  // variations
  'japanese fusion',
  'japanese free jazz',
  'japanese freejazz',
  'fusion japonês',           // Portuguese
  'free jazz japonês',        // Portuguese
  'fusion japonais',          // French
  'free jazz japonais',       // French
  'japanische fusion',        // German
  'japanischer freejazz',     // German
  'fusión japonés',           // Spanish
  'free jazz japonés',        // Spanish
  'fusion giapponese',        // Italian
  'free jazz giapponese',     // Italian
  'japanse fusion',           // Dutch
  'japanse free jazz',        // Dutch
  'japansk fusion',           // Swedish/Danish
  'japansk free jazz',        // Swedish/Danish
  'japanilainen fusion',      // Finnish
  'japanilainen free jazz',   // Finnish
  'японский джаз-фьюжн',      // Russian
  'японский фри-джаз',        // Russian
  '일본 퓨전 재즈',              // Korean
  '일본 프리 재즈',              // Korean
  '日本爵士融合',              // Chinese (Simplified)
  '日本自由爵士',              // Chinese (Simplified)
  '日本ジャズフュージョン',      // Japanese
  '日本フリージャズ',           // Japanese
  
  // Japanese jazz direct mentions
  'japanese jazz',
  'jazz japonês',      // Portuguese
  'jazz japonais',     // French
  'japanischer jazz',  // German
  'jazz japonés',      // Spanish
  'jazz giapponese',   // Italian
  'japanse jazz',      // Dutch
  'japansk jazz',      // Swedish/Danish
  'japanilainen jazz', // Finnish
  'японский джаз',     // Russian
  '일본 재즈',           // Korean
  '日本爵士',           // Chinese (Simplified)
  '日本ジャズ',         // Japanese
]

// Japanese jazz artists (names in English and Japanese)
export const ARTISTS = [
  // Bands
  // 'casiopea',
  // 'カシオペア', // Casiopea in Japanese
  // 'acid mothers temple',
  // 'アシッドマザーズテンプル', // Acid Mothers Temple in Japanese
  // 'koenjihyakkei',

  // Percussionists
  'masahiko togashi',
  // '富樫雅彦', // Masahiko Togashi in Japanese
  'akira ishikawa',
  // '石川晶', // Akira Ishikawa in Japanese
  'ryojiro furusawa',
  '古澤良治郎', // Ryojiro Furusawa in Japanese
  'george kawaguchi',
  // 'ジョージ川口', // George Kawaguchi in Japanese
  // '川口ジョージ', // George Kawaguchi's Japanese name
  'hideo shiraki',
  '白木秀雄', // Hideo Shiraki in Japanese
  'takeshi inomata',
  '猪俣猛', // Takeshi Inomata in Japanese
  'takeo moriyama',
  '森山威男', // Takeo Moriyama in Japanese
  // 'tatsuya yoshida',
  // '吉田達也', // Tatsuya Yoshida in Japanese

  // Guitarists
  'masayuki takayanagi',
  '高柳昌行', // Masayuki Takayanagi in Japanese
  'kiyoshi sugimoto',
  '杉本喜代志', // Kiyoshi Sugimoto in Japanese
  'masayoshi takanaka',
  '高中正義', // Masayoshi Takanaka in Japanese
  
  // Conductors
  'toshiyuki miyama',
  '宮間利之', // Toshiyuki Miyama in Japanese
  'miho hazama',
  '挾間美帆', // Miho Hazama in Japanese
  'nobuo hara',
  '原信夫', // Nobuo Hara in Japanese
  
  // Big Band
  'Toshiyuki Miyama & The New Herd',
  'Toshiyuki Miyama And His New Herd',
  'Toshiyuki Miyama & His New Herd',
  'Nobuo Hara & His Sharps & Flats',
  
  // Pianists
  'toshiko akiyoshi',
  '秋吉敏子', // Toshiko Akiyoshi in Japanese
  'masabumi kikuchi',
  '菊地雅章', // Masabumi Kikuchi in Japanese
  'ryo fukui',
  '福居良', // Ryo Fukui in Japanese
  'yosuke yamashita',
  '山下洋輔', // Yosuke Yamashita in Japanese
  'hiromi uehara',
  '上原ひろみ', // Hiromi Uehara in Japanese
  'kunihiko sugano',
  '菅野邦彦', // Kunihiko Sugano in Japanese
  'fumio itabashi',
  '板橋文夫', // Fumio Itabashi in Japanese
  'aki takase',
  '高瀬アキ', // Aki Takase in Japanese
  'satoko fujii',
  '藤井郷子', // Satoko Fujii in Japanese
  'hideo ichikawa',
  '市川秀男', // Hideo Ichikawa in Japanese
  'takehiro honda',
  '本多俊之', // Takehiro Honda in Japanese
  'tsuyoshi yamamoto',
  '山本剛', // Tsuyoshi Yamamoto in Japanese
  'junko kimura',
  '木村純子', // Junko Kimura in Japanese
  'masahiko satoh',
  '佐藤允彦', // Masahiko Satoh in Japanese
  
  // Trombonists
  'shigeharu mukai',
  '向井滋春', // Shigeharu Mukai in Japanese
  'hiroshi suzuki',
  '鈴木宏昌', // Hiroshi Suzuki in Japanese
  
  // Saxophonists
  'kaoru abe',
  '阿部薫', // Kaoru Abe in Japanese
  'takeru muraoka',
  '村岡建', // Takeru Muraoka in Japanese
  'akira sakata',
  '坂田明', // Akira Sakata in Japanese
  'akira miyazawa',
  '宮沢昭', // Akira Miyazawa in Japanese
  'hidehiko matsumoto',
  '松本英彦', // Hidehiko Matsumoto in Japanese
  'sleepy matsumoto',
  'sadao watanabe',
  '渡辺貞夫', // Sadao Watanabe in Japanese
  'jiro inagaki',
  '稲垣次郎', // Jiro Inagaki in Japanese

  // Bassists
  'motoharu yoshizawa',
  '吉沢元治', // Motoharu Yoshizawa in Japanese
  'masanaga harada',
  '原田政長', // Masanaga Harada in Japanese
  'hideto kanai',
  '金井英人', // Hideto Kanai in Japanese
  'keiki midorikawa',
  '緑川敬樹', // Keiki Midorikawa in Japanese
  'isao suzuki',
  '鈴木勲', // Isao Suzuki in Japanese
  
  // Trumpeters
  'terumasa hino',
  '日野皓正', // Terumasa Hino in Japanese
  'itaru oki',
  '沖至', // Itaru Oki in Japanese
  'shunzo ohno',
  '大野俊三', // Shunzo Ohno in Japanese
]

// Japanese jazz record labels
export const LABELS = [
  'deep jazz reality',
  // 'east wind',
  // 'king records',
  // 'nippon columbia',
  'takt jazz series',
  // 'teichiku records',
  // 'three blind mice',
]

/**
 * Function to check if a post is related to Japanese jazz
 * @param text The text content to check
 * @returns true if the text contains Japanese jazz related content
 */
export function isJapaneseJazzPost(text: string): boolean {
  const lowerText = text.toLowerCase()
  
  // Check for hashtags
  if (HASHTAGS.some(tag => lowerText.includes(tag.toLowerCase()))) {
    return true
  }
  
  // Check for Japanese keywords
  if (JAPANESE_KEYWORDS.some(keyword => text.includes(keyword))) {
    return true
  }
  
  // Check for artists
  if (ARTISTS.some(artist => lowerText.includes(artist.toLowerCase()))) {
    return true
  }
  
  // Check for labels
  if (LABELS.some(label => lowerText.includes(label.toLowerCase()))) {
    return true
  }
  
  return false
}
